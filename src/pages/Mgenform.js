...
function Mgenform({setNavbar, props}) {
  ...
  const formSubmit = (event) =>{
    ...
    const data = {leadName: name, leadPhoneNumber: phoneNumber};
     axios.post(`/send-message/${session_client}`, data).then( async (response) =>{
    ...
    useEffect(() => {
      axios.get(`/mgen/${session_client}`).then((response) => {
...
<div className="card">
  <div className="card-body">
    <p className="card-text text-center">{formObject.form_body}</p>
  </div>
